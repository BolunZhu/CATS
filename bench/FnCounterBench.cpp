...
void fn(TxThread * tx, uint32_t input_index,uint32_t output_index){
    uint64_t addr= TM_GET_INPUT(input_index);
    uint64_t* ct = reinterpret_cast<uint64_t*>(addr);
    TM_WRITE(*ct, 1 + TM_READ(*ct));
    // std::cout << "F"<<std::endl;
}
...
bench_test(uintptr_t, uint32_t*)
{
    TM_BEGIN(atomic) {
        ...
        TM_DELAY(
            TM_INPUT(reinterpret_cast<uint64_t>(&counter)),
            TM_OUTPUT(reinterpret_cast<uint64_t>(&counter)), 
